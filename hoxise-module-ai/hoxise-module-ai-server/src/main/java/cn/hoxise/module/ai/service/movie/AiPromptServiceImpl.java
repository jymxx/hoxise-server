...
public class AiPromptServiceImpl implements AiPromptService{
    ...
    public String buildMovieRecommendPrompt(){
        //获取所有db数据
        List<BangumiDbRespDTO> list = bangumiDbApi.page(1, 9999).getCheckedData().getList();
    ...
    public String buildMovieSummaryPrompt(Long catalogId){
        ...
        List<BangumiDbRespDTO> checkedData = bangumiDbApi.list(Collections.singletonList(catalogId)).getCheckedData();
        if (checkedData.isEmpty()){
            throw new ServiceException("未找到数据");
        }
        BangumiDbRespDTO bangumiDb = checkedData.getFirst();
        ...
        //角色数据
        List<BangumiDbCharacterRespDTO> characterList = characterApi.list(Collections.singletonList(catalogId)).getCheckedData();
